    ...
    boxsize = NULL, ...)
{
    ...
    drawNormalCI <- function(LL, OR, UL, size, testcol) {
        size = 0.75 * size
        ...
        if (clipupper || cliplower) {
            ...
            if (!clipbox)
                grid.rect(x = unit(OR, "native"), width = unit(size,
                  "snpc"), height = unit(size, "snpc"), gp = gpar(fill = testcol,
                  col = testcol))
        }
        else {
    ...
    for (i in 1:nr) {
        ...
        else drawNormalCI(lower[i], mean[i], upper[i], info[i], col$box[i])
        popViewport()
    }
    popViewport()
}
